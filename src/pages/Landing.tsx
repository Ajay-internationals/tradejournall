...
import {
    ...
    ArrowRight, Lock, RotateCcw, BarChart
} from 'lucide-react';
                ...
                <div className="max-w-6xl mx-auto text-center">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8 md:mb-16">
                        ...
                        <button
                            ...
                            className="w-full sm:w-auto px-10 py-5 bg-white text-slate-900 border border-slate-200 rounded-2xl font-bold transition-all hover:bg-slate-50 hover:border-slate-300 flex items-center justify-center gap-3"
                        >
                            Explore Features
                        </button>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="inline-block p-1 px-1 bg-white border border-slate-200 rounded-[2rem] shadow-sm">
                            <div className="px-10 py-5 bg-indigo-600 rounded-[1.8rem] text-white">
                                <p className="text-xl font-bold">Trade Adhyayan is the solution.</p>
                            </div>
                        </div>
                    </div>
            ...
            <footer className="py-20 px-6 bg-white border-t border-slate-100">
                <div className="max-w-7xl mx-auto">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
                        ...
                        <div>
                            <h4 className="font-bold text-slate-900 mb-6 font-heading">Connect</h4>
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                <p className="text-sm font-bold text-slate-900">Live in Indian Nodes</p>
                            </div>
                            <p className="text-xs text-slate-500">Made with ❤️ in India.</p>
                        </div>
                    </div>
                    <div className="pt-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
                        <p className="text-xs text-slate-400 font-bold">Trade Adhyayan © 2026</p>
...
function PricingCard({ title, price, period, description, features, isFeatured = false }: any) {
    ...
    return (
        <div className={cn(
            ...
            {isFeatured && <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-6 py-1.5 bg-indigo-600 text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">Most Popular</div>}

            <h4 className={cn("text-xs font-black uppercase tracking-[0.2em] mb-4", isFeatured ? "text-indigo-400" : "text-indigo-600")}>{title}</h4>
            <div className="flex items-baseline gap-2 mb-4 leading-none">
                <span className="text-5xl font-black">{price}</span>
                {period && <span className={cn("text-sm font-bold", isFeatured ? "text-slate-400" : "text-slate-500 tracking-wide")}>{period}</span>}
            </div>
            <p className={cn("text-sm font-medium mb-10", isFeatured ? "text-slate-400" : "text-slate-600")}>{description}</p>
