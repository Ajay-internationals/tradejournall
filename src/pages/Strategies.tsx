            ...
            <div className="relative z-10 flex flex-col h-full leading-none">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        <h3 className="text-2xl font-black tracking-tighter text-slate-900 uppercase">{name}</h3>
                    </div>
                    {stats.profitFactor >= 2 && (
                        <div className="px-3 py-1 bg-emerald-500 text-white rounded-lg shadow-xl shadow-emerald-500/20 animate-pulse">
                            <span className="text-[8px] font-black uppercase tracking-widest">Master Edge</span>
                        </div>
                    )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-6 py-8 border-t border-indigo-500/10">
                    <div className="space-y-2">
                        <p className="text-[9px] font-black text-indigo-500/50 uppercase tracking-widest">Alpha (%)</p>
                        <p className={cn(
                    ...
                    <div className="space-y-2">
                        <p className="text-[9px] font-black text-indigo-500/50 uppercase tracking-widest">P Factor</p>
                        <p className={cn(
                            "text-xl font-black tracking-tighter",
                ...
                <div className="flex items-center justify-between pt-6 gap-6">
                    <span className="text-[9px] font-black text-indigo-500/40 uppercase tracking-widest">1:{stats.avgRR.toFixed(2)} R:R Delta</span>
                    <div className="flex gap-4">
                        <button
                            onClick={() => setShowTrades(!showTrades)}
                            className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl"
                        >
                            <Activity size={12} />
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 text-[9px] font-black text-indigo-500 uppercase tracking-widest hover:text-indigo-800 transition-colors"
                        >
                            <Share2 size={12} />
                            Share
                        </button>
                ...
                {showTrades && (
                    <div className="mt-8 pt-8 border-t border-indigo-500/10 space-y-4 max-h-96 overflow-y-auto no-scrollbar animate-in slide-in-from-top-4 duration-500">
                        <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-500/30 mb-6 text-center">Protocol Execution Log ({strategyTrades.length})</h4>
                            ...
                            <div key={trade.id} className="p-6 bg-indigo-500/5 rounded-3xl border border-indigo-500/10 hover:border-indigo-500/30 transition-all shadow-inner group/item">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-5">
                                        <div className={cn(
                                            "w-10 h-10 rounded-xl flex items-center justify-center text-[11px] font-black shadow-lg",
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-[9px] text-indigo-500/30 font-black uppercase tracking-widest">Qty: {trade.quantity}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-6 text-[9px] font-black uppercase tracking-widest text-indigo-500/40">
