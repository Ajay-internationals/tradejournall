...
export default function Journal() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 font-['Quicksand'] bg-[var(--app-bg)] min-h-screen p-6 md:p-10">
            {/* Header Card */}
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-8 bg-white dark:bg-slate-900 border border-indigo-100 dark:border-slate-800 rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)]">
                <div className="text-center md:text-left">
                    <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-indigo-950 dark:text-white leading-none mb-2">Trade Journal</h1>
                    <p className="text-indigo-500/80 dark:text-slate-400 font-medium text-sm">Systematic record of your trading performance.</p>
                </div>
            ...
            <div className="flex flex-col xl:flex-row gap-6">
                <div className="flex-1 relative group">
                    ...
                    <input
                        ...
                        className="w-full bg-white dark:bg-slate-900 border border-indigo-50 dark:border-slate-800 rounded-3xl py-4 pl-14 pr-8 text-sm font-bold text-indigo-900 dark:text-white placeholder:text-indigo-300 focus:ring-4 focus:ring-indigo-50 transition-all shadow-[0_2px_10px_rgb(0,0,0,0.02)] outline-none"
                    />
                </div>
                <div className="flex gap-4">
                    <div className="relative">
                        <select
                            ...
                            className="px-8 py-4 bg-white dark:bg-slate-900 border border-indigo-50 dark:border-slate-800 rounded-3xl text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest outline-none focus:ring-4 focus:ring-indigo-50 transition-all cursor-pointer appearance-none shadow-[0_2px_10px_rgb(0,0,0,0.02)] min-w-[160px]"
                        >
                        ...
                        <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-indigo-300 pointer-events-none" size={14} />
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-3">
                {filteredTrades.length === 0 ? (
                    <div className="p-20 text-center space-y-6 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-indigo-100 dark:border-slate-800">
                        <div className="w-16 h-16 bg-indigo-50 rounded-2xl flex items-center justify-center mx-auto text-indigo-300">
                            <StickyNote size={24} />
                        </div>
                        <div>
                            <p className="text-indigo-900 dark:text-white font-bold text-lg">Journal is empty</p>
                            <p className="text-indigo-400 text-sm mt-1">Start by logging your first trade setup.</p>
                        </div>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 gap-3">
                        {/* Header Row (Hidden on mobile) */}
                        <div className="hidden md:grid grid-cols-12 gap-4 px-8 py-2 text-[10px] font-bold text-indigo-400 uppercase tracking-widest">
                        ...
                        {filteredTrades.map((trade) => (
                            <div key={trade.id} className="group bg-white dark:bg-slate-900 p-1 rounded-3xl hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all border border-indigo-50/50 dark:border-slate-800 hover:border-indigo-100 dark:hover:border-slate-700">
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center p-4">
                                    ...
                                    <div className="col-span-4 flex items-center gap-5">
                                        ...
                                        <div>
                                            <h3 className="text-base font-bold text-indigo-950 dark:text-white leading-tight">{trade.instrument}</h3>
                                            <div className="flex items-center gap-2 mt-1">
                                                ...
                                                <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">{trade.asset_class}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="col-span-2 text-center hidden md:block">
                                        <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-indigo-100/50 dark:border-slate-700 pointer-events-none">
                                            {trade.strategy}
                                        </span>
                                    </div>
                                    ...
                                    <div className="col-span-2 flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => handleEdit(trade)} className="w-9 h-9 flex items-center justify-center bg-indigo-50 text-indigo-400 rounded-xl hover:bg-indigo-500 hover:text-white transition-all"><Edit3 size={14} /></button>
                                        <button onClick={() => handleDelete(trade.id)} className="w-9 h-9 flex items-center justify-center bg-rose-50 text-rose-400 rounded-xl hover:bg-rose-500 hover:text-white transition-all"><Trash2 size={14} /></button>
                                    </div>
                                </div>
                            </div>
                        ))}
