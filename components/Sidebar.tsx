...
import { 
  LayoutDashboard, List, PlusCircle, Calendar, BarChart2, Target, BrainCircuit, Globe, Ruler, Trophy, BookOpen, Wrench, Users, Settings, LogOut, Menu, X, Zap, AlertTriangle, Map, Github, Sparkles
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ user, onLogout, currentPage, onNavigate }) => {
  ...
  const menuGroups = [
    ...
    },
    {
      title: "Intelligence",
      items: [
        { name: 'Voice Coach', path: '/live-coach', icon: Sparkles, badge: 'NEW' },
        { name: 'Reports', path: '/reports', icon: BarChart2 },
        { name: 'Strategies', path: '/strategies', icon: Target },
        { name: 'Pattern AI', path: '/ai-insight', icon: BrainCircuit },
      ]
    },
    {
      title: "Growth",
  ...
  return (
    <>
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 z-50 flex items-center justify-between px-6 backdrop-blur-md bg-opacity-90 dark:bg-opacity-90">
        <div className="flex items-center space-x-2">
           <Zap className="h-6 w-6 text-indigo-600 fill-current" />
           <span className="font-black text-xl text-slate-900 dark:text-white tracking-tighter">Adhyayan</span>
        </div>
        <div className="flex items-center gap-4">
          <button onClick={() => onNavigate('/live-coach')} className="p-2 text-indigo-600 bg-indigo-50 dark:bg-indigo-900/30 rounded-xl">
            <Sparkles className="h-5 w-5" />
          </button>
          <button onClick={() => setIsOpen(!isOpen)} className="p-2 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors">
            {isOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
          </button>
        </div>
      </div>

      <div className={`fixed inset-y-0 left-0 z-[60] w-72 bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transform transition-transform duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:static md:h-screen flex flex-col shadow-2xl md:shadow-none`}>
        ...
        <nav className="flex-1 px-4 space-y-8 overflow-y-auto pb-6 pt-20 md:pt-0 custom-scrollbar">
                ...
                {group.items.map((item) => {
                   ...
                   return (
                     ...
                     >
                       ...
                       {item.badge ? (
                         <span className={`${item.badge === 'NEW' ? 'bg-indigo-600' : 'bg-rose-500'} text-white text-[9px] font-black px-1.5 py-0.5 rounded-full`}>
                           {item.badge}
                         </span>
      ...
      {isOpen && <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 md:hidden" onClick={() => setIsOpen(false)} />}
    </>
  );
};
